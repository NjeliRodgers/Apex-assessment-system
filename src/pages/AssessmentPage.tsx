...
import {
  ApexAssessmentQuestion,
  ApexAssessmentSubmitResponse,
  getMyExamQuestionsApi,
  submitMyExamApi
} from '../api/apexAssessmentApi';
...
export const AssessmentPage: React.FC = () => {
  const { logout } = useApexAuth();
  const [examName, setExamName] = useState('');
  const [questions, setQuestions] = useState<ApexAssessmentQuestion[]>([]);
  const [answers, setAnswers] = useState<Record<string, number>>({});
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [completed, setCompleted] = useState(false);
  const [result, setResult] = useState<ApexAssessmentSubmitResponse | null>(null);
  ...
  useEffect(() => {
    ...
    getMyExamQuestionsApi()
      ...
      .catch((err: Error) => {
        if (stillMounted) {
          setError(err.message || 'Could not load your assessment.');
        }
      })
      .finally(() => {
  ...
  const finalizeSubmit = async () => {
    ...
    try {
      const data = await submitMyExamApi(answers);
      setResult(data);
      setCompleted(true);
    } catch (err: any) {
      setError(err.message || 'Your assessment could not be submitted. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const submitAssessment = async () => {
    ...
    const confirmed = window.confirm(
      'Submit your assessment now? You will not be able to change your answers after submission.'
    );
      ...
      <div className="min-h-screen bg-fog flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-lg border border-line p-8 text-center space-y-4">
          ...
          <h1 className="font-display text-2xl font-bold text-ink">
            Your interview test has been sent and delivered
          </h1>
          ...
          <p className="text-sm text-muted leading-relaxed">
            {passed
              ? 'Congratulations — you have met the pass mark. Check your email for confirmation and details on your next stage, the AI-agent interview.'
              : result && result.attemptsRemaining > 0
              ? 'You did not meet the pass mark this time. Check your email — you have been given another chance to retake the test.'
              : 'You did not meet the pass mark. Check your email for confirmation. Our team will be in touch regarding next steps.'}
          </p>
    ...
    <div className="min-h-screen bg-fog p-4 sm:p-8">
      <main className="mx-auto max-w-3xl space-y-5">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted mt-2">
                Answer every question, then review your selections before submitting.
              </p>
            </div>
        ...
        {questions.map((question, questionIndex) => (
          <section
            key={question.id}
            className="bg-white rounded-lg border border-line p-6 space-y-4"
          >
                ...
                return (
                  <label
                    ...
                    className={`flex items-start gap-3 rounded-lg border p-3 text-sm cursor-pointer transition ${
                      selected
                        ? 'border-brand-600 bg-emerald-50 text-ink'
                        : 'border-line hover:border-brand-300 text-ink/90'
                    }`}
                  >
                    <input
                      type="radio"
        ...
        <div className="bg-white rounded-lg border border-line p-5 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
          <p className="text-xs text-muted">
            You can submit only after answering every question.
          </p>
