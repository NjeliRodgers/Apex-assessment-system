...
import { BookOpen, CalendarDays, CheckCircle2, Lock, LoaderCircle, KeyRound, CreditCard, Sparkles, Layers, BarChart3 } from 'lucide-react';
import {
  getCatalogItemApi,
  getMyEnrollmentsApi,
  enrollApi,
      ...
      const handler = window.PaystackPop.setup({
        ...
        callback: (response) => {
          verifyEnrollmentPaymentApi(pendingEnrollment.id, response.reference)
            .then(async (updated) => {
              setEnrollments((prev) => [...prev.filter((e) => e.id !== updated.id), updated]);
      ...
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-lg border border-line shadow-[0_1px_2px_rgba(15,85,53,0.06)] p-6 space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-mono text-[11px] font-semibold tracking-[0.15em] text-brand-600 uppercase">Training Progress Board</p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="rounded-lg border border-line p-3 bg-fog">
                  <p className="text-[11px] uppercase tracking-wide text-muted">Course Status</p>
                  <p className="text-sm font-bold text-ink mt-1">{summary.courses.completed}/{summary.courses.total} completed</p>
                </div>
                <div className="rounded-lg border border-line p-3 bg-fog">
                  <p className="text-[11px] uppercase tracking-wide text-muted">Exam Readiness</p>
                  <p className="text-sm font-bold text-ink mt-1">{summary.exams.paid}/{summary.exams.total} paid</p>
                </div>
                <div className="rounded-lg border border-line p-3 bg-fog">
                  <p className="text-[11px] uppercase tracking-wide text-muted">Interview Stage</p>
                  <p className="text-sm font-bold text-ink mt-1">{interviewStatus}</p>
                </div>
              </div>
            </div>
          ) : (
